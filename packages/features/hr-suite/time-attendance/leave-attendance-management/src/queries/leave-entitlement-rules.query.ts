...
import type {
  ...
  ListLamLeaveEntitlementRulesQuery,
} from "../contracts/index.ts";
import { listLamLeaveEntitlementRulesQuerySchema } from "../contracts/index.ts";
...
const matchesScopeField = (
  ruleScope: LamLeaveEntitlementRule["scope"],
  field: keyof NonNullable<LamLeaveEntitlementRule["scope"]>,
...
export async function listLamLeaveEntitlementRulesRecords(
  query: ListLamLeaveEntitlementRulesQuery = {},
  context?: LamReadContext
): Promise<readonly LamLeaveEntitlementRule[]> {
  const parsed = listLamLeaveEntitlementRulesQuerySchema.parse(query);
  ...
  return paginate(
    filterByCompany(state.leaveEntitlementRules, ctx.companyId)
      .filter((entry) =>
      ...
      .filter((entry) =>
        matchesSearch(
          [
            ...
            entry.title,
            entry.leaveTypeId,
            entry.scope?.legalEntityCode,
            entry.scope?.countryCode,
          ],
          term
        )
      )
...
export async function getLamLeaveEntitlementRuleById(
  id: string,
  context?: LamReadContext
): Promise<LamLeaveEntitlementRule | null> {
  const ctx = readContext(context);
  const state = await loadLamRepository(ctx);
  ...
  return (
    filterByCompany(state.leaveEntitlementRules, ctx.companyId).find(
      (entry) => entry.id === id
    ) ?? null
  );
}
