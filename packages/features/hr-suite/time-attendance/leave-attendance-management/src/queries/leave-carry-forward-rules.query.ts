...
import type {
  ...
  ListLamLeaveCarryForwardRulesQuery,
} from "../contracts/index.ts";
import { listLamLeaveCarryForwardRulesQuerySchema } from "../contracts/index.ts";
...
const matchesScopeField = (
  ruleScope: LamLeaveCarryForwardRule["scope"],
  field: keyof NonNullable<LamLeaveCarryForwardRule["scope"]>,
...
export async function listLamLeaveCarryForwardRulesRecords(
  query: ListLamLeaveCarryForwardRulesQuery = {},
  context?: LamReadContext
): Promise<readonly LamLeaveCarryForwardRule[]> {
  const parsed = listLamLeaveCarryForwardRulesQuerySchema.parse(query);
  ...
  return paginate(
    filterByCompany(state.leaveCarryForwardRules, ctx.companyId)
      .filter((entry) =>
...
export async function getLamLeaveCarryForwardRuleById(
  id: string,
  context?: LamReadContext
): Promise<LamLeaveCarryForwardRule | null> {
  const ctx = readContext(context);
  const state = await loadLamRepository(ctx);
  ...
  return (
    filterByCompany(state.leaveCarryForwardRules, ctx.companyId).find(
      (entry) => entry.id === id
    ) ?? null
  );
}
