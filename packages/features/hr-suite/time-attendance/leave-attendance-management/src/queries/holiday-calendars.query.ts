...
import type {
  LamHolidayCalendar,
  ListLamHolidayCalendarsQuery,
} from "../contracts/index.ts";
import { listLamHolidayCalendarsQuerySchema } from "../contracts/index.ts";
...
const matchesScopeField = (
  ruleScope: LamHolidayCalendar["scope"],
  field: keyof NonNullable<LamHolidayCalendar["scope"]>,
...
export async function listLamHolidayCalendarsRecords(
  query: ListLamHolidayCalendarsQuery = {},
  context?: LamReadContext
): Promise<readonly LamHolidayCalendar[]> {
  const parsed = listLamHolidayCalendarsQuerySchema.parse(query);
  ...
  return paginate(
    filterByCompany(state.holidayCalendars, ctx.companyId)
      .filter((entry) =>
...
export async function getLamHolidayCalendarById(
  id: string,
  context?: LamReadContext
): Promise<LamHolidayCalendar | null> {
  const ctx = readContext(context);
  const state = await loadLamRepository(ctx);
  ...
  return (
    filterByCompany(state.holidayCalendars, ctx.companyId).find(
      (entry) => entry.id === id
    ) ?? null
  );
}
