...
export type EmployeeLifecycleRepositoryState = {
  ...
  notificationIntents: EmployeeLifecycleNotificationIntent[];
  offboardingHandoffs: EmployeeLifecycleOffboardingHandoffRecord[];
};
...
const employeeLifecycleRepositoryStateSchema = z.object({
  ...
  notificationIntents: employeeLifecycleNotificationIntentSchema
    .array()
    .default([]),
  offboardingHandoffs: employeeLifecycleOffboardingHandoffRecordSchema
    .array()
    .default([]),
});
...
type EmployeeLifecycleRepositoryRuntimeState = {
  cache: EmployeeLifecycleRepositoryState | null;
  repositoryFilePath: string;
};

const employeeLifecycleRepositoryStateKey = Symbol.for(
...
const filterScopedState = (
  ...
  return {
    ...
    notificationIntents: state.notificationIntents.filter((entry) =>
      matchesScope(entry, scope)
    ),
    offboardingHandoffs: state.offboardingHandoffs.filter((entry) =>
      matchesScope(entry, scope)
    ),
  };
};
...
const assertStateMatchesScope = (
  ...
  const invalidNotificationIntent = state.notificationIntents.find(
    (entry) => !matchesScope(entry, scope)
  );
  const invalidOffboardingHandoff = state.offboardingHandoffs.find(
    (entry) => !matchesScope(entry, scope)
  );
  if (
    invalidState ||
    ...
    invalidExitRecord ||
    invalidNotificationIntent ||
    invalidOffboardingHandoff
  ) {
    throw new Error(
...
const mergeScopedState = (
  ...
  const retainedNotificationIntents = current.notificationIntents.filter(
    (entry) => !matchesScope(entry, scope)
  );
  const retainedOffboardingHandoffs = current.offboardingHandoffs.filter(
    (entry) => !matchesScope(entry, scope)
  );
  ...
  return {
    ...
    notificationIntents: [
      ...retainedNotificationIntents,
      ...next.notificationIntents,
    ],
    offboardingHandoffs: [
      ...retainedOffboardingHandoffs,
      ...next.offboardingHandoffs,
    ],
  };
};
...
const loadRepositoryState = (): EmployeeLifecycleRepositoryState => {
  if (runtimeState.cache === null) {
    runtimeState.cache = readStateFromDisk();
  }
...
const saveRepositoryState = (state: EmployeeLifecycleRepositoryState): void => {
  ...
  persistState(runtimeState.cache);
};
...
export const findEmployeeLifecycleNotificationIntentByDedupeKey = (
  dedupeKey: string,
  scope?: EmployeeLifecycleRepositoryScope
): EmployeeLifecycleNotificationIntent | null => {
  const repositoryState = loadEmployeeLifecycleRepository(scope);
  return (
    repositoryState.notificationIntents.find(
      (record) => record.dedupeKey === dedupeKey
    ) ?? null
  );
};

export const listEmployeeLifecycleNotificationIntents = (
  scope?: EmployeeLifecycleRepositoryScope
): readonly EmployeeLifecycleNotificationIntent[] =>
  loadEmployeeLifecycleRepository(scope).notificationIntents;

export const upsertEmployeeLifecycleNotificationIntent = (
  nextRecord: EmployeeLifecycleNotificationIntent,
  scope?: EmployeeLifecycleRepositoryScope
): EmployeeLifecycleNotificationIntent => {
  if (scope && !matchesScope(nextRecord, scope)) {
    throw new Error(
      "Employee lifecycle notification intent does not match the requested scope."
    );
  }

  mutateEmployeeLifecycleRepository((draft) => {
    const index = draft.notificationIntents.findIndex(
      (record) => record.dedupeKey === nextRecord.dedupeKey
    );

    if (index < 0) {
      draft.notificationIntents = [...draft.notificationIntents, nextRecord];
      return;
    }

    const nextRecords = [...draft.notificationIntents];
    nextRecords[index] = nextRecord;
    draft.notificationIntents = nextRecords;
  }, scope);

  return nextRecord;
};

export const findEmployeeLifecycleOffboardingHandoffByDedupeKey = (
  dedupeKey: string,
  scope?: EmployeeLifecycleRepositoryScope
): EmployeeLifecycleOffboardingHandoffRecord | null => {
  const repositoryState = loadEmployeeLifecycleRepository(scope);
  return (
    repositoryState.offboardingHandoffs.find(
      (record) => record.dedupeKey === dedupeKey
    ) ?? null
  );
};

export const listEmployeeLifecycleOffboardingHandoffs = (
  scope?: EmployeeLifecycleRepositoryScope
): readonly EmployeeLifecycleOffboardingHandoffRecord[] =>
  loadEmployeeLifecycleRepository(scope).offboardingHandoffs;

export const upsertEmployeeLifecycleOffboardingHandoff = (
  nextRecord: EmployeeLifecycleOffboardingHandoffRecord,
  scope?: EmployeeLifecycleRepositoryScope
): EmployeeLifecycleOffboardingHandoffRecord => {
  if (scope && !matchesScope(nextRecord, scope)) {
    throw new Error(
      "Employee lifecycle offboarding handoff does not match the requested scope."
    );
  }

  mutateEmployeeLifecycleRepository((draft) => {
    const index = draft.offboardingHandoffs.findIndex(
      (record) => record.dedupeKey === nextRecord.dedupeKey
    );

    if (index < 0) {
      draft.offboardingHandoffs = [...draft.offboardingHandoffs, nextRecord];
      return;
    }

    const nextRecords = [...draft.offboardingHandoffs];
    nextRecords[index] = nextRecord;
    draft.offboardingHandoffs = nextRecords;
  }, scope);
...
export const employeeLifecycleRepositoryAuditEventSchema = z.object({
  ...
  event: z.enum([
    ...
    employeeLifecycleManagementAuditEvents.integrationChanged,
    employeeLifecycleManagementAuditEvents.notificationEnqueued,
  ]),
