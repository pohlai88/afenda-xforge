...
const KanbanContext = React.createContext<KanbanContextProps<unknown> | null>(
  null,
);
...
function KanbanItem({
  ...
  const { activeId, isColumn, onItemClick, getItemId, columns, findContainer } =
    useKanbanContext();
  const isItemDragging = activeId ? !isColumn(activeId) : false;
  const container = findContainer(value);
  const item =
    container != null
      ? columns[container].find((candidate) => getItemId(candidate) === value)
      : undefined;
