  ...
  () => (root, result) => {
    for (const selector of [":root", ".dark"]) {
      root.walkRules(selector, (rule) => {
        // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: validates CSS custom property token sources
        rule.walkDecls((decl) => {
...
const previewCssPipeline = stylelint.createPlugin(
  ...
  () => (root, result) => {
    ...
    root.walkAtRules((atRule) => {
      ...
      if (
        atRule.name === "utility" &&
        atRule.params.trim() === "sb-intro-grid-bg"
      ) {
