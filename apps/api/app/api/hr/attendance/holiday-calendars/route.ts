import type { UpsertLamHolidayCalendarInput } from "@repo/features-time-attendance-leave-attendance-management/contract";
import {
...
export async function GET(request: Request) {
  const data = await listLamHolidayCalendarsRecords(
    getQuery(request),
...
export async function POST(request: Request) {
  ...
  const result = await upsertLamHolidayCalendar(
    body as UpsertLamHolidayCalendarInput,
