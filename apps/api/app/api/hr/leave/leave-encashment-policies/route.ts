import type { UpsertLamLeaveEncashmentPolicyInput } from "@repo/features-time-attendance-leave-attendance-management/contract";
import {
...
export async function GET(request: Request) {
  const data = await listLamLeaveEncashmentPoliciesRecords(
    getQuery(request),
...
export async function POST(request: Request) {
  ...
  const result = await upsertLamLeaveEncashmentPolicy(
    body as UpsertLamLeaveEncashmentPolicyInput,
