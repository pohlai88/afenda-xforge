...
let runtimeTenantAccessOverride: (() => Promise<RuntimeTenantAccess>) | null =
  null;

export const setRuntimeTenantAccessForTesting = (
  override: (() => Promise<RuntimeTenantAccess>) | null
): void => {
  runtimeTenantAccessOverride = override;
};

export const resetRuntimeTenantAccessForTesting = (): void => {
  runtimeTenantAccessOverride = null;
};

export const resolveRuntimeTenantAccess =
  async (): Promise<RuntimeTenantAccess> => {
    if (runtimeTenantAccessOverride) {
      return runtimeTenantAccessOverride();
    }
...
export const resolveOptionalCompanyId = async (): Promise<string | undefined> => {
  if (runtimeTenantAccessOverride) {
    const access = await runtimeTenantAccessOverride();

    if (access.companyId?.trim()) {
      return access.companyId.trim();
    }
  }
