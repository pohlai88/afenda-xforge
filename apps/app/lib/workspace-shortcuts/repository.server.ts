...
export const readWorkspaceShortcuts = async (
  ...
  const [tenantPolicyRow, userPreferencesRow] = await Promise.all([
    timeDatabaseQuery(
      () =>
        database.query.tenantKeyboardShortcutPolicies.findFirst({
    ...
    timeDatabaseQuery(
      () =>
        database.query.userWorkspacePreferences.findFirst({
          where: and(
...
export const upsertUserShortcutOverrides = async (
  ...
  return readWorkspaceShortcuts(tenantId, userId);
};
...
export const upsertTenantKeyboardShortcutPolicy = async (
  ...
  const existing = await readTenantKeyboardShortcutPolicy(tenantId);
  ...
  return readTenantKeyboardShortcutPolicy(tenantId);
};
