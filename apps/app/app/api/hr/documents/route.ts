...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    return NextResponse.json(await listDocumentsForTenant(query));
  } catch (error) {
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const createdDocument = await registerDocumentForTenant(
    ...
    return mapApiRouteError(error, "Invalid document upload payload");
  }
}
