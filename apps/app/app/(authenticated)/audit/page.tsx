...
export default async function AuditPage(): Promise<ReactElement> {
  ...
  if (audit.status === "error") {
    return (
      <AuthenticatedFeatureScope featureId={AUDIT_FEATURE_ID}>
        <MetadataStateBoundary error={audit.message} state="error" />
      </AuthenticatedFeatureScope>
    );
  }
      ...
      <section className="space-y-8">
          ...
          {latestEvent ? (
            <section className="mt-8 rounded-[var(--radius-xl)] border border-border bg-background/80 p-6 shadow-sm">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                ...
                <StatusBadge
                  tone={resolveOutcomeBadgeTone(latestEvent.outcome)}
                >
                  {latestEvent.outcome}
                </StatusBadge>
              </div>
        ...
        <section className="space-y-4 rounded-[var(--radius-xl)] border border-border bg-card/95 p-6 shadow-sm">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground text-sm">
              {distinctActions} distinct action
              {distinctActions === 1 ? "" : "s"} across {data.events.length}{" "}
              loaded row
