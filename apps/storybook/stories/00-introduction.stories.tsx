    ...
    <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-10 p-6 md:p-10">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <article className="rounded-[calc(var(--radius)+0.5rem)] border bg-card p-5 shadow-sm">
          ...
          <h2 className="mt-2 font-semibold text-xl">System orientation</h2>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <article className="rounded-[calc(var(--radius)+0.5rem)] border bg-card p-6 shadow-sm">
          ...
          <ul className="mt-4 space-y-3 text-muted-foreground text-sm leading-6">
            <li>
              The story catalog stays focused on the shared package surface and
              app-facing controls.
            </li>
            <li>Global toolbar controls switch light or dark mode.</li>
            <li>
              The preview CSS bootstraps Tailwind locally so Storybook remains
              isolated from app runtime assumptions.
            </li>
          </ul>
        </article>

        <article className="rounded-[calc(var(--radius)+0.5rem)] border bg-card p-6 shadow-sm">
          <h2 className="font-semibold text-xl">Recommended next steps</h2>
          <ul className="mt-4 space-y-3 text-muted-foreground text-sm leading-6">
            <li>
              Promote shared UI metadata cards into fully rendered demo stories
              category by category.
            </li>
            <li>
              Add visual regression or interaction tests once the stable story
              set is agreed.
            </li>
            <li>
              Split oversized catalogs into finer stories if bundle size becomes
              a practical problem.
            </li>
          </ul>
        </article>
